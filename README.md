...
预计在下次较大的更新中加入一个简单的自动战斗逻辑，适用一些不需要特意索敌，瞎砍也能砍到怪的场景(~~就是不用YOLO.jpg~~)，并且0编程基础也可以自行编写连招并使用（~~绝对不是因为我懒得给每个角色都写一遍连招~~）  
~~可能会先鸽一个月~~
...
3. 对于萌新玩家，在 **游园街·餐厅** 任务首次使用前，需要在 `custom_task_config\restaurant\player_status.json` 内配置`"level"`中各个厨具的等级，所有等级默认为3，因此老玩家不用特意修改
4. 由于~~翻了半天代码也看不出是啥~~的原因，**游园街·餐厅**任务自动上架的菜品售卖时间可能会与配置时间产生一定偏差（例如配置时间是24小时，但实际售卖时间可能是22到30小时不等），由于该任务附带自动下架旧菜品的功能，建议选择的配置时间至少大于实际运行间隔的1.2倍
---
## 图形化界面
